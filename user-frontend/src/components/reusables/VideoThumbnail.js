  ...
  return (
    <div
      className={`video-thumbnail-container${
        listOrientation && listOrientation !== "row" ? "--column" : "--row"
      }${
        listOrientation === "row" && listCount && listCount === 3 ? "-3" : ""
      }${
        (listOrientation === "row" && listCount == undefined) || listCount === 4
          ? "-4"
          : ""
      }`}
    >
      <Link to={"/video/" + id} className="video-thumbnail-container__content">
        <img
          ...
          className="video-thumbnail-container__thumbnail-img"
          alt="Video Thumbnail"
        />
        <img
          src={Play}
          alt="Play Button"
          className="video-thumbnail-container__play-button"
        />
