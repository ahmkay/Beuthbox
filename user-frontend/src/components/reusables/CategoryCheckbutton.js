...
function CategoryCheckbutton({category, isActive}) {
    ...
    return (
        <div className={`category-checkbutton category-checkbutton--${category} ${active && 'category-checkbutton--isActive'}`} onClick={() => toggleIsActive()}>
            {category === 'study' && <ColorLensIcon className='category-checkbutton__img' />}
            {category === 'campus' && <SchoolIcon className='category-checkbutton__img' />}                
            {category === 'class' && <SvgIcon component={BookIcon} viewBox="0 -4 24 24" className='MuiSvgIcon-root svg-icon category-checkbutton__img' />} {/* correct viewBox to center the icon */}
            {category === 'research' && <SvgIcon component={MagicIcon} className='MuiSvgIcon-root svg-icon category-checkbutton__img' />}
