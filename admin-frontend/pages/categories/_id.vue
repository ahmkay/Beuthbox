    ...
    <v-flex xs8>
      <v-card>
        ...
        <v-card-title primary-title>
          <div class="headline">Kategorie {{name}}</div>
        </v-card-title>

        <v-card-text>
          <form ref="myForm" enctype="multipart/form-data">
            <v-text-field v-model="name" name="name" label="Name"></v-text-field>
            <v-text-field v-model="description" name="description" label="Beschreibung" multi-line></v-text-field>
            ...
            <v-layout row align-center class="mt-3">
              ...
              <v-flex>
                <v-text-field @click.native="onFocus('smallImage')" label="Avatar Icon" v-model="iconfilename" 
...
<script>
  ...
  export default {
    ...
    created() {
      this.$store.commit("settoolbarTitle", `Kategory ${this.name}`);
    },
    ...
    async asyncData({ params, error }) {
      try {
        ...
        return {
          ...
          bigImage:
          process.env.apiUrl  + "/category" + data.data.category.imagepath,
          smallImage:
          process.env.apiUrl  + "/category" + data.data.category.iconpath,
